...
def initialize_config() -> dict[str, object]:
    ...
    default_language = (
        "en" if "en" in available_languages else (available_languages[0] if available_languages else "en")
    )
    ...
    if updated:
        ...
        with open(json_file_path, "w", encoding="utf-8") as json_file:
            json_file.write(formatted_json)
        logging.info("JSON file updated with missing default variables and COLOR_CONFIG items.")
