...
from contact.ui.nav_utils import draw_arrows, move_highlight, update_help_window
    ...
    for idx, option in enumerate(menu_state.current_menu):
        ...
        if full_key.startswith("config.network.ipv4_config.") and option in {"ip", "gateway", "subnet", "dns"}:
            if isinstance(current_value, int):
                try:
                    current_value = str(ipaddress.IPv4Address(int(current_value).to_bytes(4, "little", signed=False)))
            ...
            if selected_option == "Exit":
                ...
                if new_value is not None:
                    ...
                    if overwrite == "Yes":
                        ...
                        logging.info("New Config URL sent to node")
                menu_state.start_index.pop()
                continue
                ...
                if confirmation == "Yes":
                    ...
                    logging.info("Node Reboot Requested by menu")
                menu_state.start_index.pop()
                continue
                ...
                if confirmation == "Yes":
                    ...
                    logging.info("Node DB Reset Requested by menu")
                menu_state.start_index.pop()
                continue
                ...
                if confirmation == "Yes":
                    ...
                    logging.info("Node Shutdown Requested by menu")
                menu_state.start_index.pop()
                continue
                ...
                if confirmation == "Yes":
                    ...
                    logging.info("Factory Reset Requested by menu")
                menu_state.start_index.pop()
                continue
...
def set_region(interface: object) -> None:
    ...
    new_region_name = get_list_input(t("ui.prompt.select_region", default="Select your region:"), "UNSET", regions)
